...
De acuerdo con las instrucciones del laboratorio, el objetivo de este inciso fue **comparar dos muestras provenientes de una distribución Normal(μ, σ²)** con el fin de verificar si ambas pueden considerarse estadísticamente equivalentes.
...
#### Interpretación de los resultados (α = 0.05)

- Si **p > 0.05**, **no se rechaza H₀**, lo que indica que ambas muestras pueden provenir de la misma distribución.  
- Si **p ≤ 0.05**, se **rechaza H₀**, indicando diferencias significativas entre las muestras.

En este caso:
- La **prueba de Chi–Cuadrado** arrojó un valor p = 0.0000, por lo que **se rechaza H₀**, señalando diferencias en las frecuencias.  
- La **prueba de Kolmogorov–Smirnov**, con p = 0.2202, **no rechaza H₀**, lo que sugiere que las distribuciones acumuladas son estadísticamente equivalentes.
...
La prueba de Kolmogorov–Smirnov, junto con la similitud visual del histograma y las medias casi idénticas, indica que **ambas muestras pueden considerarse provenientes de la misma distribución Normal(μ, σ²)**.  
...
Sin embargo, la **prueba de Kolmogorov–Smirnov (K–S)** dio un p-valor de **0.2202**, mayor que el nivel de significancia α = 0.05, por lo que **no se rechaza la hipótesis nula**.  
...
**Conclusión final:**  
Las dos muestras pueden considerarse **provenientes de la misma distribución Normal(μ, σ²)**.  
